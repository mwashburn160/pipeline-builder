 ...
 * and each deploy target's services.txt, so neither keeps its own hand-written
 * list.
 */
...
const REPO_FIXTURE_INPUTS: Record<string, string[]> = {
  ...
  // The schema/RLS suites boot PGlite from the docker target's init script.
  '@pipeline-builder/pipeline-data': ['{workspaceRoot}/deploy/local/docker/postgres-init.sql'],
...
// deploy/<target>/services.txt — the list the deploy scripts read
// (sync-image-tags.sh, service-signing-keys.sh, verify-npm-deps.sh):
//   <kind> <name> <dir>   kind = service (backend image) | frontend | library
//
// One copy per deploy target, like every other file a target needs: a target
// tree is standalone, so a script running for it resolves the list from that
// target and nothing outside it. The copies are generated from this one list
// and held byte-identical by test/deploy-contracts/test/bringup-contract.test.ts.
for (const target of ['deploy/local/docker', 'deploy/local/minikube', 'deploy/aws/ec2', 'deploy/aws/eks']) {
  new TextFile(root, `${target}/services.txt`, {
    lines: [
