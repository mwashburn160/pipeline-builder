...
GitGuardian enterprise secret detection plugin for scanning code repositories for hardcoded credentials and API keys using AWS CDK CodeBuildStep
...
|------|----------|-------------|
| `GITGUARDIAN_API_KEY` | Yes | GitGuardian API key |
...
```bash
aws secretsmanager create-secret --name "GITGUARDIAN_API_KEY" --secret-string "<your-value>"
```
...
{
  "secrets": {
    "GITGUARDIAN_API_KEY": "arn:aws:secretsmanager:<region>:<account>:secret:GITGUARDIAN_API_KEY"
  }
}
```
...
|----------|---------|-------------|
| `GG_SCAN_TYPE` | `path` | Gg Scan Type |
| `GG_EXIT_ZERO` | `false` | Gg Exit Zero |

## Output

Primary output directory: `gitguardian-reports`
