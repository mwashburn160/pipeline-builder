# SOC 2 Common Criteria (Starter)

Advanced content add-on — `set:advanced`, `framework:soc2`.
...
`set:advanced` + `framework:soc2` + `control:<id>`. The framework policy under
`policies/soc2/policy.json` groups them all.
