...
Advanced content add-on — `set:advanced`, `framework:pci-dss`.
...
`set:advanced` + `framework:pci-dss` + `control:<id>`. The framework policy under
`policies/pci-dss/policy.json` groups them all.
