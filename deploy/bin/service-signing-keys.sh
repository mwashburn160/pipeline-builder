...
#
# cert_dir is REQUIRED and is the deploy target's certs directory
# (deploy/local/docker/certs, deploy/aws/eks/certs, …): its parent is the target
# whose services.txt lists the identities to generate keys for, so the keys and
# the service list always come from the same tree. Everything is written to a
...
[ -n "$CERT_DIR" ] || { echo "ERROR: cert_dir is required — pass the deploy target's certs directory, e.g. deploy/aws/eks/certs" >&2; exit 1; }

# Every internal identity that signs tokens: each backend service in the OWNING
# TARGET's services.txt (generated by projen from the project definitions into
# every deploy target). cert_dir is that target's certs/ directory, so its
# parent is the target whose keys are being generated — the script never reaches
# outside the tree it is provisioning. The services take their name from
# SERVICE_NAME (identical in every deploy target), so these strings are the ones
# in `sub: service:<name>` and in the mesh policies.
TARGET_DIR="$(cd "$(dirname "$CERT_DIR")" 2>/dev/null && pwd)" \
  || { echo "ERROR: cert_dir '$CERT_DIR' is not inside a deploy target" >&2; exit 1; }
SERVICES_TXT="$TARGET_DIR/services.txt"
[ -f "$SERVICES_TXT" ] || { echo "ERROR: $SERVICES_TXT not found — cert_dir must be <deploy target>/certs (e.g. deploy/aws/eks/certs)" >&2; exit 1; }
SERVICES=()
while read -r _kind _name _dir; do
  if [ "$_kind" = service ]; then SERVICES+=("$_name"); fi
done < <(grep -v '^#' "$SERVICES_TXT")
# Generating no keys is not a success: an empty list would write a bundle with
# no service keys in it and every service would then fail to verify its peers.
[ "${#SERVICES[@]}" -gt 0 ] || { echo "ERROR: no service rows in $SERVICES_TXT — refusing to write an empty key bundle" >&2; exit 1; }
# Not a service: the deploy's own bootstrap identity, used by push-base-images.sh
# / build-plugin-images.sh to authenticate their base-image pushes against the
# in-cluster registry. It runs from an operator's shell, so its key stays on the
# host and is never mounted into a pod.
SERVICES+=(deploy-bootstrap)
