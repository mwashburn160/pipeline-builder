...
# Every internal identity that signs tokens: each backend service in
# deploy/shared/services.txt (generated by projen from the project definitions).
# The services take their name from SERVICE_NAME (identical in every deploy
# target), so these strings are the ones in `sub: service:<name>` and in the mesh
# policies.
SERVICES=()
while read -r _kind _name _dir; do
  if [ "$_kind" = service ]; then SERVICES+=("$_name"); fi
done < <(grep -v '^#' "$(dirname "${BASH_SOURCE[0]}")/../shared/services.txt")
