...
- **Env** — every target's `.env.example` is GENERATED from [`shared/env/env.example.in`](shared/env/env.example.in) by `node scripts/gen-env-examples.mjs` (CI runs `--check`); edit the template, never a copy.

## Shared config ([`shared/`](shared/))

The files that are identical in every target live **once**, in `deploy/shared/`: `postgres-init.sql` (schema + RLS), `mongodb-init.js`, the njs modules `nginx/jwt.js` + `nginx/metrics.js`, and `config/{loki/loki-config.yml, alertmanager/alertmanager.yml, thanos/objstore.yml}`. docker-compose mounts them by relative path (`../../shared/…`); the kubectl targets build their ConfigMaps from them via `pb_shared_dir` / `pb_create_config_maps` in `bin/k8s-resources.sh`. Edit them there — there are no per-target copies to keep in sync. Everything else stays in each target's own tree — the three k8s targets keep standalone manifest trees (no shared kustomize base), and their observability and gateway configs are per-target copies. Where copies must match, a deploy contract test ([`test/deploy-contracts`](../test/deploy-contracts/)) holds them equal: alert rules and promtail across the k8s targets, the Grafana dashboards across all four, the AWS gateways' `nginx.conf` / `admin-uis-disabled.conf` / `registry-auth.js`, and the per-org namespace templates. `nginx.conf` across all targets is drift-checked region by region (`nginx-drift.test.ts`), allowing differences only inside declared target-specific regions.
