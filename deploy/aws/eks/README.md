...
deploy/aws/eks/
  ...
  postgres-init.sql      # schema + RLS (→ the postgres-init ConfigMap)
  services.txt           # generated by projen: the services this target signs keys for
...
loki/alertmanager/thanos-objstore configs are this target's OWN copies, read
straight out of this tree by `pb_create_config_maps` in
`deploy/bin/k8s-resources.sh`. They are identical in every target, and
