...
describe.each(K8S_TARGETS)('network contract — %s', (target) => {
  ...
  it('gives Alertmanager public 443 egress so Slack notifications leave the cluster (D1)', () => {
  ...
  it('opens the external legs each service needs (D5)', () => {
...
describe('network contract — eks backup + NetworkPolicy enforcement', () => {
  ...
  it('lets the backup CronJob reach every datastore it dumps, and S3 (D9)', () => {
  ...
  it('mirrors every MinIO bucket the stack creates, including plugin-quarantine (D9)', () => {
  ...
  it('turns the VPC CNI network-policy controller on — Auto Mode ignores NetworkPolicy otherwise (D4)', () => {
  ...
  it('probes that a denied connection is actually denied after the apply (D4)', () => {
    const setup = read('deploy/aws/eks/bin/setup.sh');
