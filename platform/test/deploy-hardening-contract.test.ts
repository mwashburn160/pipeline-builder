...
describe('release images (D7, D10)', () => {
...
describe('local HTTPS without a CA baked into images (D7)', () => {
...
describe('ec2 stack durability + boot secrets (D8)', () => {
...
describe('data-path HA on the AWS targets (D11)', () => {
...
describe('toolchain pins (D12)', () => {
  ...
  it.each(['deploy/aws/eks/bin/setup.sh', 'deploy/aws/ec2/bin/startup.sh', 'deploy/local/minikube/bin/setup.sh'])('%s: KEDA is current', (f) => {
    expect(read(f)).toContain('kedacore/keda/releases/download/v2.20.2/');
  });
});

describe('release supply chain (D10)', () => {
  ...
  it('pins every action to a commit SHA', () => {
    for (const wf of [release, test, read('.github/workflows/deploy-configs.yml'), read('.github/workflows/deploy-shellcheck.yml'),
