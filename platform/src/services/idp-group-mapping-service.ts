 ...
 * uses to turn the groups on a validated id_token into a Role set. SCIM (3b)
 ...
 *   - WHERE it applies — every query is filtered by `orgId`, so a mapping only
...
export class IdpGroupMappingService {
  ...
  async list(orgId: string): Promise<IdpGroupMappingDto[]> {
    const docs = await IdpGroupMapping.find({ orgId }).sort({ groupKey: 1 });
  ...
  async create(
    ...
    if (await IdpGroupMapping.exists({ orgId, groupKey: key })) throw new Error(IGM_GROUP_TAKEN);
    if (await IdpGroupMapping.countDocuments({ orgId }) >= MAX_MAPPINGS_PER_ORG) throw new Error(IGM_LIMIT);
  ...
  async update(
    ...
    const doc = await IdpGroupMapping.findOne({ _id: mappingId, orgId });
    ...
    if (input.group !== undefined) {
      const group = input.group.trim();
      const key = groupKey(group);
      if (key !== doc.groupKey && await IdpGroupMapping.exists({ orgId, groupKey: key, _id: { $ne: doc._id } })) {
  ...
  async delete(orgId: string, mappingId: string, actor: RoleAssignmentActor): Promise<void> {
    const doc = await IdpGroupMapping.findOne({ _id: mappingId, orgId }).select('roleIds');
    if (!doc) throw new Error(IGM_NOT_FOUND);
    await assertMappableRoleSet(orgId, (doc.roleIds ?? []).map(String), actor);
  ...
  async resolveMappedRoles(orgId: string, groups: readonly string[]): Promise<{
    ...
    const docs = await IdpGroupMapping.find({ orgId, groupKey: { $in: keys } })
