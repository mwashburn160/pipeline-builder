...
/**
 * Social OAuth providers (Google, GitHub, Facebook, Microsoft, GitLab,
 * LinkedIn): the authorization-code flow with PKCE, the one-time CSRF state
 * bound to the browser that started it, and provider-verified identity. Shared
 * by sign-in, invitation acceptance and step-up re-auth, so none of them can
 * accept an identity the provider did not verify.
 */
...
// Redis is what lets the initiate + callback land on different replicas (a
// process-local Map alone fails most logins once there are several).
...
export const OAUTH_ERROR_MAP = {
  ...
  // A social sign-in is single-factor, so it cannot open a session in an
...
/**
 * Start a sign-in through `providerName`: mint the CSRF state (bound to the
 * browser via `binding`, see helpers/login-binding.ts) and the PKCE verifier,
 * and return where to send the browser. `error` names why the provider can't be
 * used.
 */
export async function startOAuthSignIn(
  ...
  const provider = getProvider(providerName);
  if (!provider) return { error: `Unsupported OAuth provider: ${providerName}` };
  if (!provider.enabled) return { error: `${providerName} OAuth is not configured` };

  const state = crypto.randomBytes(32).toString('hex');
  const { url, codeVerifier } = beginAuthorize(provider, state, 'sign-in');
  await pendingOAuthStates.put(state, { provider: providerName, binding: bindBrowser(), ...(codeVerifier && { codeVerifier }) });
  return { url, state };
}

/** The providers configured in this deployment. */
export function enabledOAuthProviders(): OAuthProviderName[] {
  return (Object.entries(providers) as Array<[OAuthProviderName, OAuthProvider]>).filter(([, p]) => p.enabled).map(([name]) => name);
}
