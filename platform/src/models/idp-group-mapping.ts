...
/**
 * Per-org IdP GROUP → Role mapping (3a).
 *
 ...
 * SCIM (3b) provisions Groups through the same mapping, and its per-group writes
 * would otherwise contend on one config document. Every field is scoped by
 * `orgId`, so a mapping can only ever name Roles of — and grant membership in —
...
export interface IdpGroupMappingDocument extends Document {
  /** Org whose IdP asserts this group. Matches `OrgIdpConfig.orgId` (string id). */
  orgId: string;
  ...
  groupKey: string;
  /** Roles granted to a member of this group, all owned by `orgId`. */
  roleIds: Types.ObjectId[];
  /**
   * The directory's own id for the group, as SCIM's `externalId` (3b). Set when
...
const idpGroupMappingSchema = new Schema<IdpGroupMappingDocument>(
  {
...
idpGroupMappingSchema.index({ orgId: 1, groupKey: 1 }, { unique: true });
...
idpGroupMappingSchema.index({ orgId: 1, scimExternalId: 1 }, { sparse: true });

export default model<IdpGroupMappingDocument>('IdpGroupMapping', idpGroupMappingSchema);
