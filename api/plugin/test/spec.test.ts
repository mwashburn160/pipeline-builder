...
import { parsePluginZip, validateBuildArgs } from '../src/helpers/spec';
...
describe('parsePluginZip', () => {
  it('should parse a valid ZIP with spec.yaml', async () => {
    const spec = `
name: my-plugin
version: "1.0.0"
commands:
...
`;
    ...
    expect(result.spec.name).toBe('my-plugin');
    expect(result.spec.version).toBe('1.0.0');
    expect(result.spec.commands).toEqual(['npm run build']);
  ...
  it('should throw when spec.yaml is missing', async () => {
    const zipPath = buildZip({
      'Dockerfile': 'FROM node:20',
      'README.md': '# hello',
    });
    ...
    await expect(parsePluginZip(zipPath)).rejects.toThrow('spec.yaml file missing in ZIP');
  });

  it('should throw when required spec fields are missing', async () => {
    const zipPath = buildZip({
      'spec.yaml': 'description: incomplete spec',
    });
...
`;
    const zipPath = buildZip({ 'spec.yaml': spec });
