...
> {
  ...
  /**
   * Deploy a new plugin version as the default.
   *
   * Atomically unsets old defaults for the same plugin name within the org,
   * then upserts the new record as default. Scoped by name+org (unlike
   * the base setDefault which is org-wide).
   *
   * @param data - Plugin record data (must include orgId, name, version)
   * @param userId - User ID performing the operation
   * @returns Promise resolving to the upserted plugin
   */
  async deployVersion(
    data: PluginInsert,
    userId: string,
