 ...
 * `billing_events` collection; this client exists so the shared
 * `requirePermission` / `requireSystemAdmin` gate's `authz.denied` denials
 * (#5) reach the platform audit trail like the other services'.
 *
...
function svc(): ServiceAuditClient {
  if (!audit) audit = createServiceAuditClient('billing');
  return audit;
}
