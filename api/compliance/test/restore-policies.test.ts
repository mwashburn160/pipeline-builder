...
/**
 * Tests for routes/restore-policies (POST /:id/restore).
 *
 * Mirrors the sibling policy-audit.test.ts mocking approach. The restore route
 * loads the own-org tombstone, restores it, 404s on either miss, 400s on a
 * missing id, and emits an attributed `compliance.policy.restore` event with
 * `affectedOrgId: existing.orgId` only after the restore succeeds.
 *
...
const { createRestorePolicyRoutes } = await import('../src/routes/restore-policies.js');
...
describe('POST /:id/restore — restore emits compliance.policy.restore', () => {
  it('restores the tombstone and emits on success', async () => {
    ...
    restoreMock.mockResolvedValueOnce({ id: POLICY_ID, orgId: 'org-a', name: 'Baseline' });
    const handler = lastHandler(createRestorePolicyRoutes(), '/:id/restore', 'post');
