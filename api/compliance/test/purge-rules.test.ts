...
/**
 * Tests for routes/purge-rules (POST /:id/purge).
 *
 * Mirrors the sibling restore-rules.test.ts. The purge route loads the own-org
 * tombstone (access-control gating + name/orgId capture), hard-deletes it via the
 * service's `purgeById`, 404s on either miss, 400s on a missing id, and emits an
 * attributed `compliance.rule.purge` event with `affectedOrgId: existing.orgId`
...
describe('POST /:id/purge — purge emits compliance.rule.purge', () => {
  it('hard-deletes the tombstone and emits on success', async () => {
    ...
    purgeByIdMock.mockResolvedValueOnce(RULE_ID);
    const handler = lastHandler(createPurgeRuleRoutes(), '/:id/purge', 'post');
