...
Shared server/auth/DB/Redis settings (`PORT`, `DB_*`, `PLATFORM_BASE_URL`); Redis (`REDIS_URL` or `REDIS_SENTINELS`) is read via `@pipeline-builder/api-core` are read via `@pipeline-builder/pipeline-core`. Env vars read directly by this service:
