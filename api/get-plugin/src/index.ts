...
app.get('/', authenticateToken, async (req: TypedRequest<{}, Partial<PluginFilter>>, res: Response) => {
  ...
  try {
    ...
    if (!result) {
      log('INFO', 'No plugin found matching the criteria');
...
app.get('/:id', authenticateToken, async (req: TypedRequest, res: Response) => {
  ...
  if (!id) {
    log('ERROR', 'Plugin ID is missing');
    return res.status(400).json({ message: 'Plugin ID is required.' });
  }

  log('INFO', 'Plugin query request received', { id });
...
async function startServer(): Promise<void> {
  try {
    ...
    const server = app.listen(config.server.port, () => {
      console.log(`✅ Plugin microservice listening on port: ${config.server.port}`);
