 ...
 * service emits `message.announcement.create` (create-message.ts) and
 * `message.delete` (delete-message.ts) directly, and `getAuditClient` also backs
 * the shared `requirePermission` / `requireSystemAdmin` gate so its `authz.denied`
