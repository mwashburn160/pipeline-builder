 ...
 */
const COMPLIANCE_BYPASS = envBool('COMPLIANCE_BYPASS', false);
 ...
 */
const VALIDATE_TIMEOUT_MS = envInt('COMPLIANCE_VALIDATE_TIMEOUT_MS', 4000, { min: 1 });
...
function buildHeaders(orgId: string, authHeader: string): Record<string, string> {
  ...
  // Compliance routes authenticate the caller via the `Authorization` service
  // JWT (`requireServicePrincipal`); no plaintext header is trusted.
