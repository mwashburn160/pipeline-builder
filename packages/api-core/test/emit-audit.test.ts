...
/**
 * Tests for `emitAudit` — the cross-service audit-log boundary.
 *
...
import { emitAudit } from '../src/utils/audit.js';

/** Minimal winston double: `info` + `warn` are the only methods emitAudit uses. */
function makeLogger() {
  return {
    info: jest.fn(),
    warn: jest.fn(),
...
describe('emitAudit', () => {
  ...
  it('carries the correlation ids through, so a line can be pivoted to its request/trace', () => {
    emitAudit(logger, copyEvent({ requestId: 'req-42', traceId: 'abc123' }));
  ...
  it('lets a caller-supplied eventCategory WIN over the audit tag (documented gap)', () => {
    // `emitAudit` builds `{ eventCategory: 'audit', ...audit }` — the spread
    ...
    emitAudit(logger, copyEvent({ eventCategory: 'not-audit' } as Partial<AuditEvent>));
  ...
  describe('AWS account-id scrubbing (the persistence boundary)', () => {
    it('redacts a bare 12-digit account id anywhere in the event', () => {
      emitAudit(logger, copyEvent({ source: 'arn:aws:ecr:us-east-1:123456789012:repository/app' }));
    ...
    it('redacts an account-NAMED key wholesale, even when the value is a number', () => {
      emitAudit(logger, copyEvent({ accountId: 123456789012 } as Partial<AuditEvent>));
    ...
    it('leaves a 13-digit millisecond timestamp intact (no clipping inside longer numbers)', () => {
      emitAudit(logger, copyEvent({ details: { at: '1717171717171' } } as Partial<AuditEvent>));
  ...
  describe('best-effort delivery (an audit failure must not fail the mutation)', () => {
    it('swallows a throwing transport and falls back to a warn', () => {
      ...
      expect(() => emitAudit(logger, copyEvent())).not.toThrow();
