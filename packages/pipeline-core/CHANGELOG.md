## 1.14.16 (2026-02-10)

### 🩹 Fixes

- upgraded pipeline-core=1.14.15 ([3d4ff5e](https://github.com/mwashburn160/pipeline-builder/commit/3d4ff5e))

### ❤️ Thank You

- mark

## 1.14.15 (2026-02-10)

### 🩹 Fixes

- upgraded pipeline-data=1.14.15 ([6ebed59](https://github.com/mwashburn160/pipeline-builder/commit/6ebed59))

### ❤️ Thank You

- mark

## 1.14.14 (2026-02-10)

### 🩹 Fixes

- updated plugin manifest ([422bcda](https://github.com/mwashburn160/pipeline-builder/commit/422bcda))

### ❤️ Thank You

- mark

## 1.14.13 (2026-02-10)

### 🩹 Fixes

- upgraded pipeline-core=1.14.12 ([f8c3b22](https://github.com/mwashburn160/pipeline-builder/commit/f8c3b22))

### ❤️ Thank You

- mark

## 1.14.12 (2026-02-10)

### 🩹 Fixes

- upgraded pipeline-data=1.14.12 ([dfa6f78](https://github.com/mwashburn160/pipeline-builder/commit/dfa6f78))
- upgraded pipeline-data=1.14.12 ([ade84e3](https://github.com/mwashburn160/pipeline-builder/commit/ade84e3))

### ❤️ Thank You

- mark

## 1.14.11 (2026-02-10)

### 🩹 Fixes

- rebuild ([c53ddd5](https://github.com/mwashburn160/pipeline-builder/commit/c53ddd5))

### ❤️ Thank You

- mark

## 1.14.10 (2026-02-10)

### 🩹 Fixes

- rebuild ([6ecd1c3](https://github.com/mwashburn160/pipeline-builder/commit/6ecd1c3))

### ❤️ Thank You

- mark

## 1.14.9 (2026-02-10)

### 🩹 Fixes

- rebuild ([ec575b6](https://github.com/mwashburn160/pipeline-builder/commit/ec575b6))

### ❤️ Thank You

- mark

## 1.14.8 (2026-02-10)

### 🩹 Fixes

- rebuild ([85395b1](https://github.com/mwashburn160/pipeline-builder/commit/85395b1))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.14.8

### ❤️ Thank You

- mark

## 1.14.7 (2026-02-10)

### 🩹 Fixes

- rebuild ([7e09126](https://github.com/mwashburn160/pipeline-builder/commit/7e09126))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.14.7
- Updated @mwashburn160/api-core to 1.13.7

### ❤️ Thank You

- mark

## 1.14.6 (2026-02-10)

### 🩹 Fixes

- reversion ([9cde1a5](https://github.com/mwashburn160/pipeline-builder/commit/9cde1a5))

### ❤️ Thank You

- mark

## 1.14.5 (2026-02-09)

### 🩹 Fixes

- upgraded api-server=1.11.4 ([80fb179](https://github.com/mwashburn160/pipeline-builder/commit/80fb179))

### ❤️ Thank You

- mark

## 1.14.4 (2026-02-09)

### 🩹 Fixes

- upgraded pipeline-core=1.14.3 ([33c370f](https://github.com/mwashburn160/pipeline-builder/commit/33c370f))

### ❤️ Thank You

- mark

## 1.14.3 (2026-02-09)

### 🩹 Fixes

- upgraded pipeline-data=1.14.2 ([6eaee02](https://github.com/mwashburn160/pipeline-builder/commit/6eaee02))

### ❤️ Thank You

- mark

## 1.14.2 (2026-02-09)

### 🩹 Fixes

- upgraded api-core=1.13.1 ([63f5e53](https://github.com/mwashburn160/pipeline-builder/commit/63f5e53))

### ❤️ Thank You

- mark

## 1.14.1 (2026-02-09)

### 🩹 Fixes

- added framer-motion ([d80be91](https://github.com/mwashburn160/pipeline-builder/commit/d80be91))

### ❤️ Thank You

- mark

## 1.14.0 (2026-02-09)

This was a version bump only for @mwashburn160/pipeline-core to align it with other projects, there were no code changes.

## 1.13.1 (2026-02-09)

### 🩹 Fixes

- version ([db9ef29](https://github.com/mwashburn160/pipeline-builder/commit/db9ef29))

### ❤️ Thank You

- mark

## 1.13.0 (2026-02-09)

### 🚀 Features

- upgraded api-server=1.9.0 ([38a6664](https://github.com/mwashburn160/pipeline-builder/commit/38a6664))

### ❤️ Thank You

- mark

## 1.12.0 (2026-02-09)

### 🚀 Features

- upgraded pipeline-core=1.11.0 ([bbee6fb](https://github.com/mwashburn160/pipeline-builder/commit/bbee6fb))

### ❤️ Thank You

- mark

## 1.11.0 (2026-02-09)

### 🚀 Features

- upgraded pipeline-data=1.10.0 ([43019bc](https://github.com/mwashburn160/pipeline-builder/commit/43019bc))

### ❤️ Thank You

- mark

## 1.10.0 (2026-02-09)

### 🚀 Features

- upgraded api-core=1.8.27 ([fccf551](https://github.com/mwashburn160/pipeline-builder/commit/fccf551))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.10.0

### ❤️ Thank You

- mark

## 1.9.7 (2026-02-09)

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.9.14
- Updated @mwashburn160/api-core to 1.8.27

## 1.9.6 (2026-02-09)

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.9.13

## 1.9.5 (2026-02-09)

### 🩹 Fixes

- created strong secret ([afe5ed4](https://github.com/mwashburn160/pipeline-builder/commit/afe5ed4))

### ❤️ Thank You

- mark

## 1.9.4 (2026-02-09)

### 🩹 Fixes

- added form to frontend ([63d39e9](https://github.com/mwashburn160/pipeline-builder/commit/63d39e9))

### ❤️ Thank You

- mark

## 1.9.3 (2026-02-08)

### 🩹 Fixes

- upgraded api-server=1.6.23 ([eee2516](https://github.com/mwashburn160/pipeline-builder/commit/eee2516))

### ❤️ Thank You

- mark

## 1.9.2 (2026-02-08)

### 🩹 Fixes

- upgraded pipeline-core=1.9.1 ([291a1eb](https://github.com/mwashburn160/pipeline-builder/commit/291a1eb))

### ❤️ Thank You

- mark

## 1.9.1 (2026-02-08)

### 🩹 Fixes

- upgraded pipeline-data=1.9.8 ([48bcfac](https://github.com/mwashburn160/pipeline-builder/commit/48bcfac))

### ❤️ Thank You

- mark

## 1.9.0 (2026-02-08)

### 🚀 Features

- add custom install commands ([89e448c](https://github.com/mwashburn160/pipeline-builder/commit/89e448c))

### ❤️ Thank You

- mark

## 1.8.8 (2026-02-08)

### 🩹 Fixes

- upgraded api-server=1.6.20 ([ce1cfb5](https://github.com/mwashburn160/pipeline-builder/commit/ce1cfb5))

### ❤️ Thank You

- mark

## 1.8.7 (2026-02-08)

### 🩹 Fixes

- upgraded pipeline-core=1.8.6 ([a52d16d](https://github.com/mwashburn160/pipeline-builder/commit/a52d16d))

### ❤️ Thank You

- mark

## 1.8.6 (2026-02-08)

### 🩹 Fixes

- upgraded pipeline-data=1.9.5 ([43426b7](https://github.com/mwashburn160/pipeline-builder/commit/43426b7))

### ❤️ Thank You

- mark

## 1.8.5 (2026-02-08)

### 🩹 Fixes

- upgraded api-core=1.8.17 ([0c2f76a](https://github.com/mwashburn160/pipeline-builder/commit/0c2f76a))

### ❤️ Thank You

- mark

## 1.8.4 (2026-02-08)

### 🩹 Fixes

- code cleanup ([b38ec82](https://github.com/mwashburn160/pipeline-builder/commit/b38ec82))

### ❤️ Thank You

- mark

## 1.8.3 (2026-02-08)

### 🩹 Fixes

- upgraded api-server=1.6.15 ([8f737bf](https://github.com/mwashburn160/pipeline-builder/commit/8f737bf))

### ❤️ Thank You

- mark

## 1.8.2 (2026-02-08)

### 🩹 Fixes

- upgraded pipeline-core=1.8.1 ([0577d4e](https://github.com/mwashburn160/pipeline-builder/commit/0577d4e))

### ❤️ Thank You

- mark

## 1.8.1 (2026-02-08)

### 🩹 Fixes

- upgraded pipeline-data=1.9.0 ([f7c79d7](https://github.com/mwashburn160/pipeline-builder/commit/f7c79d7))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.9.1

### ❤️ Thank You

- mark

## 1.8.0 (2026-02-08)

### 🚀 Features

- added stages ([52f8c00](https://github.com/mwashburn160/pipeline-builder/commit/52f8c00))

### 🩹 Fixes

- updated id-generator ([3caac4a](https://github.com/mwashburn160/pipeline-builder/commit/3caac4a))

### ❤️ Thank You

- mark

## 1.7.14 (2026-02-08)

### 🩹 Fixes

- api-server version=1.6.11 ([656450e](https://github.com/mwashburn160/pipeline-builder/commit/656450e))

### ❤️ Thank You

- mark

## 1.7.13 (2026-02-08)

### 🩹 Fixes

- pipeline-core version=1.7.12 ([4afe3c1](https://github.com/mwashburn160/pipeline-builder/commit/4afe3c1))

### ❤️ Thank You

- mark

## 1.7.12 (2026-02-08)

### 🩹 Fixes

- set default role=codeBuildDefault ([b6070d6](https://github.com/mwashburn160/pipeline-builder/commit/b6070d6))

### ❤️ Thank You

- mark

## 1.7.11 (2026-02-08)

### 🩹 Fixes

- pipeline-core version=1.7.10 ([9d611b5](https://github.com/mwashburn160/pipeline-builder/commit/9d611b5))

### ❤️ Thank You

- mark

## 1.7.10 (2026-02-08)

### 🩹 Fixes

- pipeline-data version=1.8.8 ([e3a9423](https://github.com/mwashburn160/pipeline-builder/commit/e3a9423))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.8.9
- Updated @mwashburn160/api-core to 1.8.9

### ❤️ Thank You

- mark

## 1.7.9 (2026-02-08)

### 🩹 Fixes

- api-core version=1.8.7 ([f6b92d6](https://github.com/mwashburn160/pipeline-builder/commit/f6b92d6))

### ❤️ Thank You

- mark

## 1.7.8 (2026-02-08)

### 🩹 Fixes

- created default role ([143d299](https://github.com/mwashburn160/pipeline-builder/commit/143d299))

### ❤️ Thank You

- mark

## 1.7.7 (2026-02-08)

### 🩹 Fixes

- api-server version=1.6.6 ([4154d7c](https://github.com/mwashburn160/pipeline-builder/commit/4154d7c))

### ❤️ Thank You

- mark

## 1.7.6 (2026-02-08)

### 🩹 Fixes

- pipeline-core version=1.7.5 ([8d3d9e4](https://github.com/mwashburn160/pipeline-builder/commit/8d3d9e4))

### ❤️ Thank You

- mark

## 1.7.5 (2026-02-08)

### 🩹 Fixes

- pipeline-data version=1.8.4 ([37cdfe0](https://github.com/mwashburn160/pipeline-builder/commit/37cdfe0))

### ❤️ Thank You

- mark

## 1.7.4 (2026-02-08)

### 🩹 Fixes

- api-core version=1.8.3 ([ab0eec3](https://github.com/mwashburn160/pipeline-builder/commit/ab0eec3))

### ❤️ Thank You

- mark

## 1.7.3 (2026-02-08)

### 🩹 Fixes

- add proactive token refresh ([7843f58](https://github.com/mwashburn160/pipeline-builder/commit/7843f58))

### ❤️ Thank You

- mark

## 1.7.2 (2026-02-08)

### 🩹 Fixes

- api-core version=1.8.1 ([697215c](https://github.com/mwashburn160/pipeline-builder/commit/697215c))

### ❤️ Thank You

- mark

## 1.7.1 (2026-02-08)

This was a version bump only for @mwashburn160/pipeline-core to align it with other projects, there were no code changes.

## 1.7.0 (2026-02-08)

### 🚀 Features

- added ses ([d19a805](https://github.com/mwashburn160/pipeline-builder/commit/d19a805))
- added role,ssecurity groups ([628ab22](https://github.com/mwashburn160/pipeline-builder/commit/628ab22))
- updated metadata keys ([320fb4e](https://github.com/mwashburn160/pipeline-builder/commit/320fb4e))

### ❤️ Thank You

- mark

## 1.6.7 (2026-02-07)

### 🩹 Fixes

- api version=1.7.7 ([bb41041](https://github.com/mwashburn160/pipeline-builder/commit/bb41041))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.7.8
- Updated @mwashburn160/api-core to 1.7.8

### ❤️ Thank You

- mark

## 1.6.6 (2026-02-06)

### 🩹 Fixes

- api version=1.7.6 ([946b590](https://github.com/mwashburn160/pipeline-builder/commit/946b590))

### ❤️ Thank You

- mark

## 1.6.5 (2026-02-06)

### 🩹 Fixes

- re-version ([5bc06b4](https://github.com/mwashburn160/pipeline-builder/commit/5bc06b4))

### ❤️ Thank You

- mark

## 1.6.4 (2026-02-06)

### 🩹 Fixes

- upgraded pipeline data=1.7.4 ([91c5592](https://github.com/mwashburn160/pipeline-builder/commit/91c5592))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.7.5

### ❤️ Thank You

- mark

## 1.6.3 (2026-02-06)

### 🩹 Fixes

- upgraded pipeline data=1.7.3 ([d9960bf](https://github.com/mwashburn160/pipeline-builder/commit/d9960bf))

### ❤️ Thank You

- mark

## 1.6.2 (2026-02-06)

### 🩹 Fixes

- added jwt_role ([3cf1683](https://github.com/mwashburn160/pipeline-builder/commit/3cf1683))

### 🧱 Updated Dependencies

- Updated @mwashburn160/pipeline-data to 1.7.3

### ❤️ Thank You

- mark

## 1.6.1 (2026-02-06)

### 🩹 Fixes

- upgraded pipeline data=1.7.1 ([5c55e85](https://github.com/mwashburn160/pipeline-builder/commit/5c55e85))

### ❤️ Thank You

- mark

## 1.6.0 (2026-02-06)

### 🚀 Features

- added api's ([2294e43](https://github.com/mwashburn160/pipeline-builder/commit/2294e43))

### ❤️ Thank You

- mark

## 1.5.0 (2026-02-05)

### 🚀 Features

- added frontend ([58720e9](https://github.com/mwashburn160/pipeline-builder/commit/58720e9))

### ❤️ Thank You

- mark

## 1.4.0 (2026-02-05)

### 🚀 Features

- added platform ([cb68f1b](https://github.com/mwashburn160/pipeline-builder/commit/cb68f1b))

### ❤️ Thank You

- mark

## 1.3.0 (2026-02-05)

### 🚀 Features

- added pipeline-manager ([ef5775a](https://github.com/mwashburn160/pipeline-builder/commit/ef5775a))

### ❤️ Thank You

- mark

## 1.2.0 (2026-02-05)

### 🚀 Features

- added api-server ([275883b](https://github.com/mwashburn160/pipeline-builder/commit/275883b))

### ❤️ Thank You

- mark

## 1.1.0 (2026-02-05)

### 🚀 Features

- added pipeline-core ([dbd889a](https://github.com/mwashburn160/pipeline-builder/commit/dbd889a))

### ❤️ Thank You

- mark