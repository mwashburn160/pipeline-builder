...
- **RBAC** — Owner, Admin, Member base roles enforced at the API layer, per organization, plus **custom permission groups** that grant fine-grained `resource:action` permissions (e.g. `pipelines:write`, `members:manage`, `compliance:write`); a user's effective permissions are their role's base bundle unioned with every group they belong to. A parent-org admin inherits admin over its teams
