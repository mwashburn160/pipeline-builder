...
| `GET` | `/auth/sso/:orgId/saml/metadata` | **SAML** service-provider metadata (XML) for the IdP administrator: entity ID, ACS URL with the HTTP-POST binding, `WantAssertionsSigned`. Derived from the org id and the deployment URL, so it works before the connection does and leaks nothing | — (public) |
