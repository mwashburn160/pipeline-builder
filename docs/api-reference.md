...
|--------|----------|-------------|------|
| `POST` | `/auth/sso/discover` | Login-page hint: `{ email }` → `{ sso: boolean }`. Deliberately reports nothing else — it is unauthenticated, so returning the org id or provider would make it a tenant-enumeration oracle | — (pre-auth) |
