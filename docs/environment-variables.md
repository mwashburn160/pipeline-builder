...
A `docker:27-dind` sidecar runs an isolated Docker daemon with auto-generated TLS certs. Plugin builds cannot see or affect host containers.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `DOCKER_HOST` | `tcp://plugin-dind:2376` | Docker daemon address (dind sidecar). Omit when using host socket. |
| `DOCKER_TLS_VERIFY` | `1` | Enable TLS for dind connection. Set `0` or omit for host socket. |
| `DOCKER_CERT_PATH` | `/certs` | TLS certificate directory (auto-generated by dind) |
