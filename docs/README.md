...
| [Logs](observability-logs.md) | Per-organization application logs: search syntax, entry detail, raw view, download, masking |
...
A user can belong to several organizations and teams at once and acts within one at a time (switch with the org switcher). A parent-org admin can also **open** any of its teams from the switcher without being on the team — the session runs as `admin` there on the authority inherited from the parent, with no roster entry and no seat (see [Authentication](authentication.md#switching-organizations-and-parent-admin-access-to-teams)). A parent admin can delete one of its teams (restorable during the retention window from the parent's recently-deleted list); a sysadmin can move a team to another eligible root, split it out as a standalone org, or bring a team-less root in as a team.
