...
type PluginReportTab = 'overview' | 'builds' | 'versions';

const TABS: { id: PluginReportTab; label: string }[] = [
...
/** Plugin reports page with tabbed layout: Overview, Builds, and Versions. */
export default function PluginReportsPage() {
  const { user, isReady, isAuthenticated } = useAuthGuard();

  const [activeTab, setActiveTab] = useState<PluginReportTab>('overview');
  ...
  }, [activeTab, buildInterval, dateFrom, dateTo]);

  useEffect(() => {
    if (isAuthenticated) fetchTab();
  }, [isAuthenticated, fetchTab]);

  if (!isReady || !user) return <LoadingPage />;

  const hasOverviewData = pluginSummary !== null;
    ...
    <DashboardLayout
      ...
      actions={
        <div className="flex items-center gap-3">
          ...
          <AutoRefresh onRefresh={fetchTab} loading={loading} />
        </div>
      }
    >
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }} className="page-section space-y-6">

        <ReportTabs tabs={TABS} activeTab={activeTab} onTabChange={(id) => setActiveTab(id as PluginReportTab)} />

        {/* ════════════════════════ Overview Tab ════════════════════════ */}
        {activeTab === 'overview' && (
          <>
            {loading && !hasOverviewData && <StatCardSkeleton count={5} />}

            {!loading && !hasOverviewData && (
              <EmptyState
                icon={Puzzle}
                title="No plugin data yet"
                description="Create and build plugins to see inventory stats and distribution here."
        ...
        {activeTab === 'builds' && (
          <>
            ...
            {!loading && !hasBuildsData && (
              <EmptyState
                icon={Puzzle}
                title="No build data yet"
                description="Build some plugins to see success rates, durations, and failures."
                illustration="plugins"
              />
            )}
        ...
        {activeTab === 'versions' && (
          <>
            ...
            {!loading && !hasVersionsData && (
              <EmptyState
                icon={Puzzle}
                title="No version data yet"
                description="Create plugins to see version tracking and freshness warnings."
                illustration="plugins"
              />
            )}
