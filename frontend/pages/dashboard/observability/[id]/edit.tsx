...
/**
 * Dashboard editor (v1 — list-based; no drag-resize yet).
 *
 * Why list, not grid: the plan calls for `react-grid-layout` (~120 kB) to do
 * full drag-resize. We're shipping the editor surface — name, description,
 * visibility, panel add/remove/reorder/edit — without the grid library so the
 * bundle stays unchanged. Grid-layout drops into the same page in a follow-up
 * by replacing the position-input + ArrowUp/Down controls with the grid's
 * `<GridLayout onLayoutChange={…}>` driver; `layoutJson` is already in the
 * schema and round-trips through the API.
 *
...
export default function DashboardEditPage() {
  ...
  const movePanel = useCallback((index: number, delta: -1 | 1) => {
    setPanels((prev) => {
      ...
      // Renumber positions so the save payload reflects the new order.
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Drag-resize layout is a follow-up — reorder panels with the arrow buttons for now."
      actions={
