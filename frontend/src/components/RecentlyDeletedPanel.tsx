 ...
 */
export function RecentlyDeletedPanel({ resource }: { resource: Resource }) {
  ...
  return (
    <Card>
      ...
      {loading && rows.length === 0 ? (
        ...
        <p className="text-sm text-gray-400">No recently deleted {labels.plural}.</p>
      ) : (
        <div className="overflow-x-auto">
          <DataTable
            ...
            animated={false}
            getRowKey={(r) => r.id}
            emptyState={{ icon: History, title: `No recently deleted ${labels.plural}`, description: `Deleted ${labels.plural} that are still within the retention window appear here.` }}
          />
        </div>
      )}
    </Card>
