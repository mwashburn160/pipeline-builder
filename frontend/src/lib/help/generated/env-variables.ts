  ...
  "sections": [
      ...
      "blocks": [
        {
          ...
          "items": [
            ...
            "Plugin Ecosystem -- Public directory, publishing, reviews, submissions (flags)",
      ...
      "blocks": [
        {
          ...
          "rows": [
            ...
            [
              "TRUST_PROXY",
              "1",
              "Reverse-proxy hops Express trusts for req.ip. 1 on every target: nginx is the only hop a service sees, and it OVERWRITES X-Forwarded-For with the client address (on AWS that address comes from nginx real_ip, which trusts X-Forwarded-For only from the load balancer CIDRs the setup script derives — PB_TRUSTED_PROXY_CIDRS)."
            ],
            [
              "LOG_LEVEL",
              "info",
        ...
        },
        {
          ...
          "rows": [
            [
              ...
              "—",
              "Required in production (platform refuses to boot without it; ≥ 32 chars). HMAC-SHA256 key of the audit hash chain. Lives outside the database on purpose — someone with Mongo write access but not this key cannot rebuild a consistent chain after editing or deleting events. Generated by deploy/bin/gen-env-secrets.sh (secret; k8s: app-secrets)."
            ],
            [
            ...
            [
              ...
              "—",
              "Its secret; generated by gen-env-secrets.sh."
            ],
            [
            ...
            [
              ...
              "COMPLIANCE",
              "COMPLIANCE, GOVERNANCE or none (only for storage without Object Lock)."
            ],
            [
              "AUDIT_HEAD_EXPORT_RETENTION_DAYS",
              "400",
              "Retention stamped on each exported head."
            ],
            [
              "AUDIT_HEAD_EXPORT_INTERVAL_MS",
              "300000",
              "Export period (min 10000)."
            ],
            [
              "AUDIT_SPOOL_DRAIN_INTERVAL_MS",
              "30000",
        ...
        },
        {
          ...
          "rows": [
            ...
            [
              ...
              "— (generated by gen-env-secrets.sh)",
              "Password of ecosystem_public_reader, the view-only login behind the anonymous public plugin directory (SELECT on the public_listings / public_listed_versions views, nothing else). Read by postgres-init.sql (creates the role), by pgbouncer (adds it to its userlist) and by the plugin service. Empty → the role isn't created and the directory stays off."
            ],
            [
              "PUBLIC_DIRECTORY_DB_NAME",
              "pipeline_builder_public",
              "pgbouncer database alias the plugin service's directory reads use: a separate pool_size=5 pool, and ecosystem_public_reader is capped at 5 server connections across every pool ([users] max_user_connections), so unauthenticated traffic can't exhaust the tenant pools"
            ]
          ]
        },
        {
          "type": "text",
          "content": "Every Postgres client goes through pgbouncer (DB_HOST=pgbouncer, DB_PORT=6432 on every target). No service connects to Postgres directly, including the public directory's reader login. pgbouncer's userlist carries exactly two logins: the DB_USER application role and ecosystem_public_reader. The superuser is never reachable through the pooler."
        },
        {
          "type": "text",
          "content": "MongoDB"
    ...
    },
    {
      ...
      "blocks": [
        ...
        {
          "type": "text",
          "content": "Plugin-image signing (cosign) — image-registry signs, plugin verifies"
        },
        {
          "type": "text",
          "content": "Every plugin image the plugin service pushes is signed with cosign (key-based, transparency log off) and gets a signed SPDX SBOM attestation; synth pins CodeBuild to the verified digest. The private key lives only in image-registry, which signs at POST /internal/plugin-signatures (service token; the caller must be plugin). The plugin service holds only the public key: its pod shares a network namespace with the buildkitd sidecar that runs untrusted tenant Dockerfile RUN steps, so it must never hold the private key or be able to reach AWS credentials. Keys come from deploy/bin/plugin-signing-keys.sh; rotation invalidates every existing signature — see Plugin-signing key."
        },
        {
          "type": "text",
          "content": "image-registry"
        },
        {
          "type": "table",
          "headers": [
          ...
          "rows": [
            [
              "PLUGIN_SIGNING_MODE",
              "local",
              "local signs with a PEM from disk; kms keeps the private key inside AWS KMS (asymmetric ECC_NIST_P256, SIGN_VERIFY)."
            ],
            [
              "PLUGIN_SIGNING_KEY_FILE",
              "/etc/pipeline-builder/plugin-signing/plugin-signing.key",
              "local mode: the EC P-256 private key (PKCS#8 PEM), mounted from the plugin-signing-key Kubernetes Secret (image-registry only). Imported into cosign's own format once per process, under a random in-memory password."
            ],
            [
              "PLUGIN_SIGNING_KMS_KEY_ID",
              "—",
              "Required in kms mode. The KMS key, by alias (alias/pipeline-builder-plugin-signing) — an ARN embeds the AWS account id and is refused. image-registry's role (not plugin's) needs kms:Sign + kms:GetPublicKey."
            ],
            [
              "PLUGIN_SIGNING_TIMEOUT_MS",
              "120000",
              "Upper bound on one cosign invocation (sign / attest)."
            ],
            [
              "TMPDIR",
              "/tmp",
              "cosign writes its TUF cache, the imported key and each SBOM predicate under the temp dir — a writable scratch-tmp emptyDir on the k8s targets (the root filesystem is read-only)."
            ]
          ]
        },
        {
        ...
        },
        {
          ...
          "rows": [
            [
              "PLUGIN_SIGNING_PUBLIC_KEY_FILE",
              "/etc/pipeline-builder/plugin-signing/plugin-signing.pub",
              "PEM public key plugin images are verified against (cosign verify), mounted from the plugin-signing-public-key Secret. In kms mode it is exported from KMS by the deploy script."
            ]
          ]
        }
      ]
      ...
      "blocks": [
        ...
        },
        {
          ...
          "rows": [
            ...
            [
              "GRYPE_DB_CACHE_DIR",
              "$TMPDIR/grype-db (deploys set /var/cache/grype)",
              "grype's vulnerability DB for the build-time scan and the nightly rescan. Every deploy mounts a volume here (compose: the grype-db named volume; k8s: a 2 Gi emptyDir) — the plugin container's root filesystem is read-only, and a fresh DB is hundreds of MB"
            ],
            [
              "PLUGIN_GRYPE_DB_AUTO_UPDATE",
              "true",
              "Refresh the vulnerability DB (at most every PLUGIN_GRYPE_DB_UPDATE_INTERVAL_MS before a build scan, and always before the nightly rescan). false = an operator maintains the DB (air-gapped installs)"
            ],
            [
              "PLUGIN_GRYPE_DB_UPDATE_INTERVAL_MS",
              "3600000",
              "Minimum time between DB refreshes before a build scan (1 h)"
            ],
            [
              "PLUGIN_GRYPE_DB_UPDATE_TIMEOUT_MS",
              "600000",
              "Bound on one DB refresh (10 min)"
            ],
            [
              "PLUGIN_RESCAN_ENABLED",
              "true",
              "Nightly vulnerability rescan of every active image plugin's signed SBOM (leader-locked, one pod per pass). false switches it off"
            ],
            [
              "PLUGIN_RESCAN_INTERVAL_MS",
              "86400000",
              "Time between completed rescan passes (24 h); pods tick hourly and run a pass only when this has elapsed"
            ],
            [
              "PLUGIN_RESCAN_LOCK_TTL_MS",
              "21600000",
              "Rescan leader-lock TTL (6 h) — must outlast one pass"
            ],
            [
              "PLUGIN_RESCAN_STARTUP_DELAY_MS",
              "120000",
              "Delay before the first rescan tick after boot"
            ],
            [
    ...
    },
    {
      ...
      "blocks": [
        ...
        },
        {
          ...
          "rows": [
            [
              ...
              "true",
              "Public, searchable plugin directory (/plugins, /api/public/plugins*). Needs ECOSYSTEM_PUBLIC_READER_PASSWORD. Off → both return 404"
            ],
            [
              "PLUGIN_PUBLISHING_ENABLED",
              "false in the shipped .env.example files; unset → on when billing is on (hosted), off when it is off (self-hosted)",
              "Tenant orgs may submit publish requests. Every listing and version is still approved by the system org's Ecosystem Managers. Off → tenant requests answer 403 PLUGIN_PUBLISHING_DISABLED; existing listings still resolve. The system org's Official catalog is never affected. The hosted hub sets it true"
            ],
            [
              "PLUGIN_REVIEWS_ENABLED",
              "true",
              "Ratings and reviews. Off → read-only"
            ],
            [
              "ANONYMOUS_SUBMISSIONS_ENABLED",
              "false",
              "Not-logged-in plugin submissions (quarantine → isolated build → moderation). Stays unavailable without outbound email (magic-link verification)"
            ],
            [
              "SUBMISSION_POW_SECRET",
              "CHANGE_ME (generated by gen-env-secrets.sh)",
              "HMAC key that signs the self-hosted proof-of-work challenges (GET /api/public/plugin-submissions/challenge). Required while ANONYMOUS_SUBMISSIONS_ENABLED=true — the service fails closed without it. k8s: lands in app-secrets"
            ],
            [
              "SUBMISSION_POW_DIFFICULTY",
              "20",
              "Leading zero bits a PoW solution must have (20 ≈ 1M SHA-256 hashes, about a second in a browser Web Worker). Raise during a flood"
            ],
            [
              "SUBMISSION_EMAIL_HASH_SECRET",
              "CHANGE_ME (generated)",
              "HMAC key the submitter's normalized email is hashed under (email_hash: per-email rate limit, update ownership, claim matching). Rotating it orphans existing submissions' ownership. k8s: app-secrets"
            ],
            [
              "SUBMISSION_BUILD_TIMEOUT_SECONDS",
              "900",
              "Wall-clock cap on one quarantine build on the isolated buildkitd"
            ],
            [
              "SUBMISSION_MAX_ZIP_BYTES",
              "52428800 (50 MiB)",
              "Largest accepted submission zip. nginx caps the POST body at 50m on /api/public/plugin-submissions and /inspect — keep the two in step"
            ],
            [
              "SUBMISSION_EXTRACT_DIR",
              "<os tmpdir>/pb-submission-extract",
              "Scratch directory the plugin service unpacks submission zips into for inspection (bounded, cleaned per submission)"
            ],
            [
              "SUBMISSION_MAX_CONCURRENT_EXTRACTS",
              "2",
              "How many submission zips may be extracted at once (1–64) — bounds the disk/CPU an anonymous flood can claim"
            ],
            [
            ...
            [
              ...
              "tcp://buildkitd-quarantine:1234 (docker) / tcp://plugin-quarantine-builder:1234 (k8s)",
              "The isolated buildkitd quarantine builds run on: no credentials, its own network/NodePool, egress to package mirrors + the registry only. No fallback to the tenant BUILDKIT_HOST — unset means submissions fail closed"
            ],
            [
              "OFFICIAL_AUTO_APPROVAL_ENABLED",
              "true",
              "The seeded rule that auto-approves routine, gate-green patch/minor updates of existing Official listings submitted by the catalog loader. Off → every Official update waits for two-person approval"
            ],
            [
              "PUBLISHER_TERMS_VERSION",
              "2026-09-21",
              "The publisher terms version in force. Bumping it makes every publisher re-accept before its next request (PUBLISHER_TERMS_REQUIRED); existing listings are unaffected"
            ],
            [
              "ECOSYSTEM_BOOTSTRAP_WINDOW_HOURS",
              "24",
              "How long the one-time bootstrap exception stays open after the first Official request of an empty instance, so the initial catalog load (whose builds finish in parallel) is approved automatically. It closes for good when it elapses or when any manager decides a request"
            ],
            [
              "ECOSYSTEM_VULN_GATE_MAX_CRITICAL",
              "0",
              "The vulnerability gate a version must pass to be requested (and an anonymous submission to reach moderation): at most this many fixable CRITICAL findings (grype reports a fixed version) in its scan. Unfixable findings are shown but don't block"
            ],
            [
        ...
        {
          "type": "text",
          "content": "Public directory API tuning (plugin service; all optional):"
        },
        {
          "type": "table",
          "headers": [
          ...
          "rows": [
            [
              ...
              "120",
              "Requests per minute per trusted client IP on /api/public/*. The frontend's server-rendered directory pages call the API from the frontend server, so they share that server's bucket; CDN caching of the pages absorbs most of it. Raise it for a busy self-hosted directory without a CDN"
            ],
            [
              "PUBLIC_DIRECTORY_POOL_SIZE",
              "4",
              "Connections per plugin replica in the view-only reader pool. Keep it below pgbouncer's per-user cap for ecosystem_public_reader (5)"
            ],
            [
              "PUBLIC_DIRECTORY_QUERY_TIMEOUT_MS",
              "5000",
              "Client-side bound on one directory query (pgbouncer's transaction mode can't carry statement_timeout)"
            ]
          ]
        }
      ]
      ...
      "blocks": [
        {
          ...
          "rows": [
            ...
            [
              "IMAGE_REGISTRY_SERVICE_HOST",
              "image-registry",
              "image-registry API hostname — the plugin worker asks it to sign pushed images (POST /internal/plugin-signatures). Not the registry itself (IMAGE_REGISTRY_HOST)."
            ],
            [
        ...
        },
        {
          ...
          "rows": [
            ...
            [
              ...
              "120000",
              "How long an in-flight Idempotency-Key reservation holds the key if the process dies mid-request (retries get 409 until then). Must exceed the slowest handler; a client disconnect no longer releases the key — the handler's own result settles it"
            ],
            [
      ...
      "blocks": [
        {
          ...
          "rows": [
            ...
            [
              "HTTP_CLIENT_TIMEOUT",
              "5000",
              "Internal HTTP client timeout — a TOTAL deadline for the whole exchange (connect, send, full body), not just socket idle"
            ],
            [
      ...
      "blocks": [
        ...
        },
        {
          ...
          "rows": [
            ...
            [
              "QUOTA_DEFAULT_LISTINGS",
              "developer-tier value",
              "Plugin-ecosystem listing limit for an org with no tier resolved"
            ],
            [
        ...
        },
        {
          ...
          "rows": [
            ...
            [
              ...
              "2048",
              "Cap on the tokens one Ask answer may generate (min 64) — bounds the cost and latency of a single turn"
            ],
            [
              "MAX_PAGE_OFFSET",
              "100000",
