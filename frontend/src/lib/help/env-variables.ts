      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['AI_PROVIDER', 'CLI `infra provision`: provider to use (anthropic | openai | google | xai | bedrock; default anthropic)'],
            ['AI_MODEL', 'CLI `infra provision`: model id override (defaults to the provider\'s first model)'],
          ],
        },
        {
          type: 'note',
          content: 'These keys power AI pipeline/plugin generation and the `pipeline-manager infra provision` installer (natural-language goal parsing + failure diagnosis). Without a key, `infra provision` falls back to its deterministic prereq-check + command-assembly path.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['PLUGIN_DLQ_BACKOFF_BASE_MS', '300000', 'DLQ backoff base delay (5 min)'],
