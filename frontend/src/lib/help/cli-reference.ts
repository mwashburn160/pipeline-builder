      ...
      blocks: [
        {
          ...
          rows: [
            ['auth login', 'Authenticate against the platform and persist the access token (--refresh, --org)'],
            ...
            ['pipeline deploy --id <id>', 'Fetch pipeline config from Platform, run cdk deploy, and register the pipeline by pipelineId'],
            ['pipeline register --id <id>', 'Re-register a deployed pipeline and drain pending registration intents (recovery path)'],
            ['infra provision', 'Recommended installer for the PLATFORM (local/minikube/EC2/Fargate): prereq checks + assembles the exact bin/setup.sh command; --execute runs it (gated, then verifies health + init-platform). On failure it diagnoses + auto-fixes/retries known issues. --teardown removes a deployment (AWS targets need a typed confirmation; --force for CI). Flags: --target, --prompt, --execute, --yes, --retries, --teardown, --force, --no-init, --diagnose'],
            ['status --id <id>', 'Report the current deployment and execution status'],
            ['pipeline create --file <json>', 'Create a pipeline from a props JSON file'],
            ['pipeline list / pipeline get --id <id>', 'List pipelines / get one by ID'],
            ['plugin new --name <name>', 'Scaffold a local plugin directory to edit and upload'],
            ['plugin upload --file <zip>', 'Upload a plugin ZIP to the platform'],
            ['plugin validate --dir <path>', 'Validate a local plugin directory before upload'],
            ['plugin list / plugin get --id <id>', 'Browse the plugin catalog / get one by ID'],
            ['template validate', 'Validate {{ … }} templates in a pipeline or plugin spec'],
            ['infra store-token', 'Generate a long-lived JWT and store it in AWS Secrets Manager (used by the events Lambda)'],
            ['infra setup-events', 'Deploy the EventBridge → SQS → Lambda stack that streams CodePipeline events into reporting'],
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['ANTHROPIC_API_KEY (or other provider key)', 'No', 'Enables `infra provision`\'s NL goal parsing + failure diagnosis (else it falls back to the deterministic advisor)', '—'],
            ['AI_PROVIDER / AI_MODEL', 'No', 'Provider + model for `infra provision` (anthropic|openai|google|xai|bedrock)', 'anthropic'],
          ],
        },
      ],
    },
