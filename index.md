...
- **RBAC** — Owner / Admin / Member base roles enforced per organization at the API layer, plus **custom permission groups** granting fine-grained `resource:action` permissions (a user's effective permissions = their role's bundle ∪ every group they belong to); a parent-org admin inherits admin over its teams
